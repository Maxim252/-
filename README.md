...
| Бометрия     | Доверенный            | MM                                | Компрометация приводит к нарушению цели 1 и 3, так как в этом модуле циркулируют ПДн в виде биометрии |
| Система контроля платежа        | Доверенный            | SM                                | Компрометация приводит к нарушению цели 1, так как можно перехватить и отправить чужой биометрический идентификатор |
| Система управления             | Доверенный            | CL                                | Компрометация приводит к нарушению целей 1, 2, 4, потому как это главный модуль, влияние на который отразится на всю систему |
...
| Сетевой модуль           | Повышающий доверие    | MS                                      | Компрометация приводит к нарушению цели 3 и 1 (перехват био-идентификаторов или вывод канала из строя) |
